...
RegisterData Motorola68000Private::registerInfo[] = {
    ...
    {"A7 (USP)", 0xffffffff, "Address Register 7 (User Stack Pointer)"},
    {"A7 (SSP)", 0xffffffff, "Address Register 7 (Supervisor Stack Pointer)"},
...
Motorola68000Private::Motorola68000Private(Motorola68000* q)
    : q_ptr(q),
      ...
      tracing(false),
      decodeCacheTable(0),
      instructionLogFile(nullptr),
      instructionLog(nullptr)
{
    this->registerData.resize(19);
    this->decodeCacheTable = new ExecutionPointer[65536];

    this->instructionLogFile = new QFile("instruction.log", q);
    this->instructionLogFile->open(QFile::WriteOnly);
    this->instructionLog = new QTextStream(this->instructionLogFile);
...
int Motorola68000Private::poke(quint32 address, unsigned int value, int size)
{
    ...
    case LONG:
        if (this->bus->poke(address + 0, (quint8)(value >> 24)) ||
                ...
                this->bus->poke(address + 3, (quint8)(value)))
            return EXECUTE_BUS_ERROR;
...
int Motorola68000Private::ExecuteADDX(int opcode, QString& traceRecord, int trace) {
    ...
    // Get the addresses
    if (opcode & 8) {
        if ((status = this->computeEffectiveAddress(src_address, in_register_flag,
                                                    traceRecord, 0x20 | (opcode & 7), size, trace)) != EXECUTE_OK)
            return status;

        if (trace)
            traceRecord += ", ";

        if ((status = this->computeEffectiveAddress(dest_address, in_register_flag, traceRecord,
                                                    0x20 | ((opcode & 0x0e00) >> 9), size, trace)) != EXECUTE_OK)
            return status;

        if ((status = this->peek(src_address, src, size)) != EXECUTE_OK)
            return status;

        if ((status = this->peek(dest_address, dest, size)) != EXECUTE_OK)
            return status;
    } else {
        src_address = D0_INDEX + (opcode & 0x0007);
        src = this->registerData[src_address];
        dest_address = D0_INDEX + ((opcode & 0x0e00) >> 9);
        dest = this->registerData[dest_address];

        if (trace) {
            traceRecord += this->registerInfo[src_address].name;
            traceRecord += ", ";
            traceRecord += this->registerInfo[dest_address].name;
        }
    }

    if (this->registerData[SR_INDEX] & X_FLAG)
        result = src + dest + 1;
    else
        result = src + dest;
    ...
    if (opcode & 8) {
        if ((status = this->poke(dest_address, result, size)) != EXECUTE_OK)
            return status;
    } else {
        this->setRegister(dest_address, result, size);
    }
...
int Motorola68000Private::ExecuteANDItoSR(int opcode, QString& traceRecord, int trace) {
    ...
    this->setRegister(SR_INDEX, this->registerData[SR_INDEX] & src, WORD);
...
int Motorola68000Private::ExecuteCMP(int opcode, QString& traceRecord, int trace) {
    ...
    if (trace)
        traceRecord += "," +
                this->registerInfo[register_number].name +
                " " + QString::number(ea_data,16)  + "-" + QString::number(this->registerData[register_number],16);
...
int Motorola68000Private::ExecuteEOR(int opcode, QString& traceRecord, int trace) {
    ...
    if (in_register_flag)
        this->setRegister(ea_address, result, size);
    else if ((status = this->poke(ea_address, result, size)) != EXECUTE_OK)
        return status;
...
int Motorola68000Private::ExecuteEORI(int opcode, QString& traceRecord, int trace) {
    ...
    if ((status = this->computeEffectiveAddress(dest_addr, in_register, traceRecord,
                                                opcode & 0x3f, size, trace)) != EXECUTE_OK)
        return status;
...
int Motorola68000Private::ExecuteEXG(int opcode, QString& traceRecord, int trace) {
    ...
    // Get the src and dest register numbers
    switch ((opcode & 0x00f8) >> 3) {
    case 8: // Data Registers
        src_register = D0_INDEX + ((opcode & 0xe00) >> 9);
    ...
    tmp = this->registerData[src_register];
    this->setRegister(src_register, this->registerData[dest_register], LONG);
    this->setRegister(dest_register, tmp, LONG);

    if (trace)
        traceRecord += "EXG.L " + this->registerInfo[src_register].name + ", " + this->registerInfo[dest_register].name;
...
int Motorola68000Private::ExecuteLINK(int opcode, QString& traceRecord, int trace) {
    int status, in_register_flag, stack_index, register_number;
    unsigned int displacement;
    quint32 address;
    QString ea_description;

    // Get the displacement data pointer
    if ((status = this->computeEffectiveAddress(address, in_register_flag, ea_description, 0x3c, WORD, trace)) != EXECUTE_OK)
        return status;

    // Fetch the displacement data
    if ((status = this->peek(address, displacement, LONG)) != EXECUTE_OK)
        return status;
    ...
    if ((status = this->poke(this->registerData[stack_index], this->registerData[register_number], LONG)) != EXECUTE_OK)
        return status;

    // Move the stack pointer into the address register
    this->setRegister(register_number, this->registerData[stack_index], LONG);

    // Add displacement to the stack pointer
    this->setRegister(stack_index, this->registerData[stack_index] + displacement, LONG);

    if (trace)
        traceRecord += "LINK.L " + this->registerInfo[register_number].name + ", " + ea_description;
...
int Motorola68000Private::ExecuteLINKWORD(int opcode, QString& traceRecord, int trace) {
    ...
    // Fetch the displacement data
    if ((status = this->peek(address, displacement, WORD)) != EXECUTE_OK)
        return status;
    ...
    if (trace)
        traceRecord += "LINK.W " + this->registerInfo[register_number].name + ", " + ea_description;
...
int Motorola68000Private::ExecuteMOVEfromSR(int opcode, QString& traceRecord, int trace) {
    ...
    if (in_register)
        this->setRegister(address, this->registerData[SR_INDEX], WORD);
    else if ((status = this->poke(address, this->registerData[SR_INDEX], WORD)) != EXECUTE_OK)
        return status;

    if (trace)
        traceRecord += "MOVE.W SR," + ea_description;
...
int Motorola68000Private::ExecuteMOVEfromCCR(int opcode, QString& traceRecord, int trace) {
    ...
    if (in_register)
        this->setRegister(address, this->registerData[SR_INDEX] & 0x00FF, WORD);
    else if ((status = this->poke(address, this->registerData[SR_INDEX] & 0x00FF, WORD)) != EXECUTE_OK)
        return status;

    if (trace)
        traceRecord += "MOVE.W CCR," + ea_description;
...
int Motorola68000Private::ExecuteMOVEtoCCR(int opcode, QString& traceRecord, int trace) {
    ...
    this->setRegister(SR_INDEX, (this->registerData[SR_INDEX] & 0xFF00) | (data & 0x00FF), WORD);
...
int Motorola68000Private::ExecuteMULS(int opcode, QString& traceRecord, int trace) {
    ...
    if (in_register_flag)
        ea_data = this->registerData[ea_address];
    else if ((status = this->peek(ea_address, ea_data, WORD)) != EXECUTE_OK)
        return status;
    ...
    data = this->signExtend(data, WORD);
    ea_data = this->signExtend(ea_data, WORD);

    result = data * ea_data;
    this->setConditionCodes(0, 0, result, LONG, OTHER,
                            V_FLAG | C_FLAG | Z_FLAG | N_FLAG);
    this->setRegister(register_number, result, LONG);
...
int Motorola68000Private::ExecuteNEG(int opcode, QString& traceRecord, int trace) {
    ...
    data = this->signExtend(data, size);
    result = 0 - data;
    this->setConditionCodes(data, 0, result, size, SUBTRACTION,
                            C_FLAG | X_FLAG | V_FLAG | Z_FLAG | N_FLAG);

    // Store the result
    if (in_register_flag)
        this->setRegister(address, result, size);
    else if ((status = this->poke(address, result, size)) != EXECUTE_OK)
        return status;
...
int Motorola68000Private::ExecuteNOT(int opcode, QString& traceRecord, int trace) {
    ...
    // Get the effective address
    if ((status = this->computeEffectiveAddress(address, in_register_flag, traceRecord,
                                                opcode & 0x3f, size, trace)) != EXECUTE_OK)
        return status;
    ...
    result = ~data;
    this->setConditionCodes(data, 0, result, size, OTHER,
                            C_FLAG | V_FLAG | Z_FLAG | N_FLAG);
...
int Motorola68000Private::ExecutePEA(int opcode, QString& traceRecord, int trace) {
    ...
    if ((status = this->push(stack_index, address, LONG)) != EXECUTE_OK)
        return status;
...
int Motorola68000Private::ExecuteROXL(int opcode, QString& traceRecord, int trace) {
    ...
    if (size == 3) {
        ...
        if (opcode & 32) {
            shift_count = this->registerData[D0_INDEX + ((opcode & 0x0e00) >> 9)] & 0x3f;
            if (trace)
                ea_description = this->registerInfo[D0_INDEX + ((opcode & 0xe00) >> 9)].name;
        } else {
            if ((shift_count = (opcode & 0x0e00) >> 9) == 0)
                shift_count = 0;

            if (trace)
                ea_description += "#$" + QString::number(shift_count);
        }   if (trace)
            ea_description += "," + this->registerInfo[D0_INDEX + (opcode & 7)].name;
        ...
        // Set extend
        extend = this->registerData[SR_INDEX] & X_FLAG;
...
int Motorola68000Private::ExecuteSCC(int opcode, QString& traceRecord, int trace) {
    ...
    if (this->checkConditionCodes((opcode & 0x0f00) >> 8, traceRecord, trace))
        result = 0xff;
    else
        result = 0;
    ...
    // Store the result
    if (in_register_flag)
        this->setRegister(address, result, BYTE);
    else if ((status = this->poke(address, result, BYTE)) != EXECUTE_OK)
        return status;
...
int Motorola68000Private::ExecuteUNLK(int opcode, QString& traceRecord, int trace) {
    int status, stack_index, register_number;
    unsigned int fp;
    ...
    this->setRegister(stack_index, this->registerData[register_number], LONG);

    // Pop the frame pointer from the stack
    if ((status = this->peek(this->registerData[stack_index], fp, LONG)) != EXECUTE_OK)
        return status;

    this->setRegister(register_number, fp, LONG);
    this->setRegister(stack_index, this->registerData[stack_index] + 4, LONG);

    if(trace)
        traceRecord += "UNLK " + this->registerInfo[register_number].name;
